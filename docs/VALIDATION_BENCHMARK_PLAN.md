...
- optional Rust CPU benchmark는 `_metroflow_rust` 확장 빌드 후 `traffic.meso` edge batch,
  ...
  greedy single-candidate route path core, ranked-K route candidate enumeration을 대상으로 한다
...
- Rust CPU routing parity test는 baseline reverse-Dijkstra node cost-to-go, next-link action costs,
  blocked link, unreachable node, zero-link, forbidden-turn greedy path, ranked-K candidate paths를
  동일 입력으로 비교한다
- measured routing candidate benchmark는 baseline/Rust routing backend 요청이 dynamic-potential,
  greedy path, ranked-K enumeration에 전달되는지, 그리고 result metadata가 final path,
  ranked-K candidate metadata, candidate enumeration backend, requested/actual backend/fallback,
...
- routing `auto` fallback test는 Rust routing extension이 cost-to-go, next-link scoring, greedy path,
  ranked-K candidate enumeration을 모두 제공하지 않으면 Rust wrapper를 호출하지 않고 baseline으로
  내려가는지 확인한다
