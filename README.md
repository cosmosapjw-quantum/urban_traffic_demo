...
- `SimulationConfig`의 runtime backend 기본값은 `edge_backend="baseline"`,
  ...
  deterministic greedy single-candidate path core, ranked-K candidate enumeration을 담당한다.
- `route_max_candidates`는 기본값 1을 유지하지만 2 이상을 명시하면 Python baseline이
  ...
  Rust CPU copy-boundary backend로 실행할 수 있으며, baseline 결과와 parity를 유지해야 한다.
...
greedy single-candidate route path core, ranked-K route candidate enumeration을 담당한다. Python wrapper가 NumPy-compatible
...
ranked-K route candidate generation은 Python baseline을 authority로 유지하며,
