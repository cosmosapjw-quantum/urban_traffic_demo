...
pub(crate) fn evolve_edges_batch_impl(
    ...
    if num_edges >= 1024 {
        let results: Result<Vec<(f64, f64, f64)>, String> = (0..num_edges)
            .into_par_iter()
            .map(|edge_idx| {
                ...
                Ok((queue_after, stock_after, travel_time_after))
            })
            .collect();
        ...
        for (q, s, t) in items {
            next_queue.push(q);
            next_stock.push(s);
            next_travel_time.push(t);
        }
        return Ok((next_queue, next_stock, next_travel_time));
    }
