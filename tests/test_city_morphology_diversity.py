...
def test_morphology_atlas_writes_review_bundle_without_raw_osm(tmp_path) -> None:
    ...
    assert manifest["artifact_format_version"] == "city_morphology_atlas_v3"
    ...
    assert all(
        item["quality_gate"]["gate_version"] == "morphology_quality_v2"
        for item in manifest["styles"]
    )
    assert all(item["continuous_fabric_strategy"] for item in manifest["styles"])
    assert all(item["continuous_fabric_segment_count"] > 0 for item in manifest["styles"])
